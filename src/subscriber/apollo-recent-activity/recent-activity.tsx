...
export const RecentActivity = ({ activity }: {activity: RecentActivities[]}) => {
  ...
  const buildItem = (elem: any, timestamp: number, type: string) => (
    <div
      ...
      <Text className={classes.name} block>
        ...
        {elem.type === ACTIVITY_TYPE.OPERATION ? elem.data?.name : elem.data.__activity_key}
      </Text>
      ...
      <div className={classes.time}>{moment(timestamp).format('hh:mm:ss.SSS')}</div>
    </div>
  );
  ...
  const buildActivityItems = () => {
    ...
    activity.forEach((elem: any, index: number) => {
      const m = elem.mutations.map((mutation: any) => ({
        ...
        onClick: () => {setDetailsValue({...mutation, __activity_type: mutation.type, isMutation: true})},
        content: (buildItem(mutation, elem.timestamp, 'Mutation'))
      }));
      const q = elem.queries.map((query: any) => ({
        index: index + elem.timestamp,
        onClick: () => {setDetailsValue({...query, __activity_type: query.type, isMutation: false})},
        content: (buildItem(query, elem.timestamp, "Query"))
      }));
      const c = elem.cache.map((cache: any) => ({
        index: index + elem.timestamp,
        onClick: () => {setDetailsValue({...cache, __activity_type: cache.type})},
  ...
  return (
    <div className={classes.activityContainer}>
      <List 
        ...
        fill
      />
      <DetailComponent detailsValue={detailsValue} closeDetails={closeDetails}/>
    </div>
  )
};

const DetailComponent = ({detailsValue, closeDetails}: {detailsValue: any, closeDetails: () => void}) => {
  if (!detailsValue) return null

  return detailsValue.__activity_type === ACTIVITY_TYPE.OPERATION ? <ActivityDialog value={detailsValue} onClose={closeDetails}/> : <Dialog value={
    {
